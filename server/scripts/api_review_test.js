...
(async () => {
  ...
  try {
    ...
    // POST review
    const postRes = await doFetch("http://localhost:5005/api/review", {
      method: "POST",
      headers: {
    ...
    const getOrderRes = await doFetch(
      `http://localhost:5005/api/review/${productId}?orderId=${orderId}`
    );
    ...
    const getAllRes = await doFetch(
      `http://localhost:5005/api/review/${productId}`
    );
